...
public class Backup {
	...
	//private final static ThreadLocal<DateFormat> DATE_FORMAT_THREAD_LOCAL = ThreadLocal<new SimpleDateFormat("yyy-MM-dd")>; //ThreadLocal.withInitial(() -> new SimpleDateFormat("yyy-MM-dd"));
	...
	private final static ThreadLocal DATE_FORMAT_THREAD_LOCAL = new ThreadLocal(){
        protected synchronized Object initialValue() {
            return  new SimpleDateFormat(DATE_FORMAT, Locale.getDefault());
        }
    };
    
    private final static ThreadLocal DATE_TIME_FORMAT_THREAD_LOCAL = new ThreadLocal(){
        protected synchronized Object initialValue() {
            return  new SimpleDateFormat(DATE_TIME_FORMAT, Locale.getDefault());
        }
    };
    private final static ThreadLocal TIME_FORMAT_THREAD_LOCAL = new ThreadLocal(){
        protected synchronized Object initialValue() {
	...
	public String getSQLValue(Table table, Row row, int index) {
		...
		if (row.get(index) == null) {
			...
			case Types.DATE:
				return "\"" + ((DateFormat) DATE_FORMAT_THREAD_LOCAL.get()).format(row.getDate(index)) + "\"";

			case Types.TIME:
				return "\"" + ((DateFormat) TIME_FORMAT_THREAD_LOCAL.get()).format(row.getDate(index)) + "\"";

			case Types.TIMESTAMP:
				return "\"" + ((DateFormat) DATE_TIME_FORMAT_THREAD_LOCAL.get()).format(row.getDate(index)) + "\"";
